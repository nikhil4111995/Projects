...
select * from Customers where First_Name in ('Maria','Aris','James') ;
select * from Customers where First_Name LIKE 'A%';

select * from Rentals where Amount > 50;

select DATE(Pick_up_date),SUM(Amount) from Rentals 
GROUP BY (Pick_up_date)
ORDER BY SUM(Amount) DESC;

select Customer_ID,SUM(Amount) from Rentals
WHERE Customer_ID BETWEEN '1' AND '10'
GROUP BY Customer_ID 
HAVING SUM(Amount) <200;

select Car_Category.Category_ID,Model,Brand from Car_Category 
LEFT JOIN Cars
ON Car_Category.Category_ID = Cars.Category_ID
WHERE Car_Category.Category_ID in (1,2);

select Location.Location_ID,City,State,Phone_Number,Phone_ID from Location 
INNER JOIN Location_Phone_Number
ON Location.Location_ID = Location_Phone_Number.Location_ID
WHERE Location.Location_ID in (1,2,3);